...
# ====================================
echo "[STEP 1/3] Launching export_infer_xtc.sh on SDF..."

JOB_ID="$(ssh "${USER}@${DEST_HOST}" "bash -lc '
  set -euo pipefail
  cd \"${SDF_BASE}\"
    jid=\$(sbatch --parsable run_export_infer_xtc.sh RUN_NUMBER=${RUN_NUMBER} EXP_NUMBER=${EXP_NUMBER} SAVE_NORMALIZED=${SAVE_NORMALIZED} MAX_EVENTS=${MAX_EVENTS} USE_NORMALIZED=${USE_NORMALIZED} CAMERA_NAME=${CAMERA_NAME})
  echo \$jid
'")"
