# Coyote Protector XTC Workflow (Parallel, Chunked)
...
This requires loading code on both SDF and psdev and updating a few configuration values. The process is described below.
...
2) Clone the full GitHub repo, then copy only the relevant pipeline folder `coyote_protector_xtc_parallel_pipeline` to the folder you just created.

```bash
git clone git@github.com:lcls-mlcv/coyote_protector.git
cp -r coyote_protector/scripts/coyote_protector_xtc_parallel_pipeline /sdf/data/lcls/ds/mfx/exp_id/results/your/folder/to/process/images/
```
Now that the folder is on SDF, update these values in `routine_detection_v3.sh`:
 
- `SDF_BASE`: set to the folder you copied on SDF (for example `/sdf/data/lcls/ds/mfx/exp_id/results/your/folder/to/process/images/coyote_protector_xtc_parallel_pipeline`)
- `RESULTS_BACK_BASE`: set to the psdev/CDS folder where you want CSV results stored.

3) Rsync that SDF folder to CDS.
Connect to psdev and go to your CDS folder where you want final results stored, then run:
...
To avoid too many authentications, configure passwordless SSH from psdev to SDF before production runs.

-  Generate an SSH key on **psdev**:
...
- Test the connection from psdev:
